...
contract SupplyChain {
  ...
  modifier checkValue(uint _sku) {
    ...
    uint refundAmt = msg.value - _price;
    // payable(items[_sku].buyer).transfer(refundAmt);
    items[_sku].buyer.transfer(refundAmt);
  }
  ...
  modifier forSale(uint _sku) {
    ...
    require (items[_sku].sku > 0,"item not for sale");
    _;
  }
  ...
  function _payable(address _address) internal pure returns (address payable) {
    ...
    verifyCaller(items[_sku].seller) 
      {
        ...
        emit LogShipped(items[_sku].sku);
      }
  ...
  function receiveItem(uint _sku) 
      ...
      verifyCaller(items[_sku].buyer)
    {
        ...
        emit LogReceived(items[_sku].sku);
    }
